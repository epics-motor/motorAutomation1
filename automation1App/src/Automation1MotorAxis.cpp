...
asynStatus Automation1MotorAxis::poll(bool* moving)
{
    ...
    // Determine whether this axis is participating in an in-flight coordinated
    // hexapod move.  During such a move the axis's own MotionDone bit stays 1
    // and its velocityFeedback stays 0 (the trajectory is generated by the
    // hexapod task, not by the per-axis motion planner), so the normal
    // done/moving derivation below would leave motorStatusMoving_ = 0 and
    // motorStatusDone_ = 1 and would freeze the motor record's .RBV at the
    ...
    if (coordMoveActive)
    {
        // Physical feedback for both motorPosition_ and motorEncoderPosition_.
    ...
    else if (isHexapodAxis_)
    {
        // Use the commanded position as the RMP for hexapod axes at rest
    ...
    if (coordMoveActive)
    {
        // Task-driven coordinated move: force moving=1, done=0.  Direction is
        // not derivable from per-axis velocityFeedback (which is 0) so leave
        // motorStatusDirection_ unchanged.
        *moving = true;
